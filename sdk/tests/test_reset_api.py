"""The `waddle_sdk.init`/`waddle_sdk.rollout`-level reset API —
`TeleopReset`/`AgentReset`, the `pre_reset`/`post_reset` kwargs, and its
e2e pytest suite. Complements `test_reset.py` (which
drives `waddle_sdk._core` directly) one layer up: everything here goes through
the public `waddle_sdk.init`/`waddle_sdk.rollout` surface.
"""
...
def test_teleop_reset_and_agent_reset_are_frozen_and_repr_friendly():
    t = waddle_sdk.TeleopReset("reset the scene")
    ...
    a = waddle_sdk.AgentReset("clear the table", timeout_s=30.0)
    ...
    with pytest.raises(TypeError):
        waddle_sdk.TeleopReset("go", 30.0)  # type: ignore[misc]


@pytest.mark.parametrize("cls", [waddle_sdk.TeleopReset, waddle_sdk.AgentReset])
...
def test_reset_spec_kwargs_maps_markers_to_ffi_kwargs():
    assert waddle_sdk._reset_spec_kwargs("pre_reset", None) == {"pre_reset_kind": "none"}

    kwargs = waddle_sdk._reset_spec_kwargs("pre_reset", waddle_sdk.TeleopReset("go", timeout_s=1.5))
    assert kwargs == {
    ...
    kwargs = waddle_sdk._reset_spec_kwargs("post_reset", waddle_sdk.AgentReset("go2"))
    assert kwargs == {
    ...
    hook_kwargs = waddle_sdk._reset_spec_kwargs("pre_reset", lambda task: True)
    ...
    with pytest.raises(TypeError, match="pre_reset"):
        waddle_sdk._reset_spec_kwargs("pre_reset", 123)


def test_reset_override_kwargs_unset_means_inherit():
    assert waddle_sdk._reset_override_kwargs("pre_reset", waddle_sdk._UNSET) == {}
    assert waddle_sdk._reset_override_kwargs("pre_reset", None) == {"pre_reset_kind": "none"}


def test_normalize_reset_hook_passes_through_valid_shapes():
    assert waddle_sdk._normalize_reset_hook(lambda task: True)("t") == (True, True)
    assert waddle_sdk._normalize_reset_hook(lambda task: False)("t") == (False, False)
    assert waddle_sdk._normalize_reset_hook(lambda task: (True, None))("t") == (True, None)
    assert waddle_sdk._normalize_reset_hook(lambda task: (False, True))("t") == (False, True)


def test_normalize_reset_hook_raises_typeerror_naming_the_contract():
    wrapped = waddle_sdk._normalize_reset_hook(lambda task: "not a bool")
...
def test_init_rejects_bad_reset_marker():
    with pytest.raises(TypeError, match="pre_reset"):
        waddle_sdk.init("py-bad-reset", _robot(), _control(), pre_reset=123)  # type: ignore[arg-type]
...
def test_rollout_override_disables_or_replaces_session_reset(tmp_path):
    ...
    with waddle_sdk.rollout(task="ep1", post_reset=None) as ep1:
    ...
    with waddle_sdk.rollout(task="ep2", post_reset=override_post) as ep2:
...
def test_malformed_pre_reset_hook_return_is_indistinguishable_from_a_legitimate_failure(tmp_path):
    ...
    `RuntimeError` text), through the public `waddle_sdk.init`/`waddle_sdk.rollout`
...
def test_with_exit_during_post_reset_does_not_abort_it(tmp_path):
    ...
    assert ep.outcome == waddle_sdk.Outcome.SUCCESS, "the no-op __exit__ must not flip this to ABORT"
    ...
    assert ep.outcome == waddle_sdk.Outcome.SUCCESS  # never altered by post-reset completion
    waddle_sdk.shutdown()
...
def test_remote_post_reset_window_via_testing_hooks(tmp_path):
    session = waddle_sdk.init(
        ...
        _control(),
        recording_dir=tmp_path,
        post_reset=waddle_sdk.TeleopReset("reset the scene"),
        _testing=True,
    )
    episode_id = None
    with waddle_sdk.rollout(task="towel") as ep:
        ...
        waddle_sdk._testing.reset_window_engage(session, "claim-remote-1", "teleop")

        deadline = time.monotonic() + 5.0
        noop_seen = False
        while time.monotonic() < deadline:
            waddle_sdk._testing.push_teleop(session, [0.5, 0.0, 0.0])
            out = ep.gate(a)
        ...
        waddle_sdk._testing.reset_window_complete(session, "claim-remote-1", True)
        term_thread.join(timeout=5.0)
...
def test_remote_post_reset_window_timeout_sets_post_reset_failed(tmp_path):
    ...
    waddle_sdk.init(
        ...
        _control(),
        recording_dir=tmp_path,
        post_reset=waddle_sdk.TeleopReset("reset the scene", timeout_s=0.15),
        _testing=True,
    )
    with waddle_sdk.rollout(task="towel") as ep:
        episode_id = ep.id
    ...
    assert ep.outcome == waddle_sdk.Outcome.SUCCESS  # pinned, unchanged by the timeout
    assert ep.post_reset_failed
    waddle_sdk.shutdown()
...
def test_dims_mismatch_intervention_rejected_with_fault(tmp_path):
    ...
    with waddle_sdk.rollout(task="towel") as ep:
        ...
        waddle_sdk._testing.engage(session, "claim-dims-1", "teleop")
