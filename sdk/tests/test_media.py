...
def _robot(camera: waddle_sdk.Camera, n_joints: int = 3) -> waddle_sdk.Robot:
...
def test_publish_frame_round_trips_through_the_loopback_far_end():
    camera = waddle_sdk.Camera(width=4, height=4, fps=30)
    session = waddle_sdk.init("py-media-roundtrip", _robot(camera), _control(), _testing=True)
    ...
    while time.monotonic() < deadline:
        got = waddle_sdk._testing.frames(session, "overhead")
        if got:
            break
        time.sleep(0.005)
...
def test_publish_frame_wrong_dtype_raises_type_error():
    camera = waddle_sdk.Camera(width=4, height=4, fps=30)
    session = waddle_sdk.init("py-media-dtype", _robot(camera), _control(), _testing=True)
...
def test_publish_frame_wrong_shape_raises_type_error():
    camera = waddle_sdk.Camera(width=4, height=4, fps=30)
    session = waddle_sdk.init("py-media-shape", _robot(camera), _control(), _testing=True)
...
def test_publish_frame_rejects_fortran_ordered_arrays():
    ...
    camera = waddle_sdk.Camera(width=4, height=4, fps=30)
    session = waddle_sdk.init("py-media-fortran", _robot(camera), _control(), _testing=True)
...
def test_publish_frame_unknown_camera_raises():
    camera = waddle_sdk.Camera(width=4, height=4, fps=30)
    session = waddle_sdk.init("py-media-unknown", _robot(camera), _control(), _testing=True)
...
def test_publish_frame_fps_throttle_is_observable():
    camera = waddle_sdk.Camera(
        width=2,
        height=2,
        fps=30,
        stream_policy=waddle_sdk.StreamPolicy(
            uplink=waddle_sdk.Uplink(fps=5.0, encoding="rgb8")
        ),
    )
    session = waddle_sdk.init("py-media-throttle", _robot(camera), _control(), _testing=True)
    ...
    got = len(waddle_sdk._testing.frames(session, "overhead"))
