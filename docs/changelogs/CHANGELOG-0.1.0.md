...
This file preserves the coordinated `0.1.0` release of the repository.

## [0.1.0] - 2026-08-23

### Fixed

- Restore the release contract around the newly public connector compatibility
  diagnostics: the packaged SDK guide and exact root-export drift test now agree with
  `ConnectorRegistrationError` and `ConnectorCompatibilityWarning`.
- Package the repository's Apache-2.0 license text in both Python wheels and hold both
  packaging-root copies byte-for-byte to the canonical root license.
- Stop tracking the generated PyO3 extension and correct its stale pre-rename ignore
...
- Add a reusable push/pull-request CI workflow for the complete Python, Rust,
  formatting, clippy, gRPC, and LiveKit gate. The release workflow must pass that same
  gate before starting either wheel matrix, and both publish identities now wait for
  the complete default and teleop builds so an exact-pinned release is all-or-nothing.
...
- **release pipeline (`.github/workflows/release.yml`, `docs/RELEASING.md`)**: the
  ...
  **two publish jobs over two environments** (`publish-sdk` → `pypi`,
  ...
  pending-publisher field values, and what to do when a leg fails. The reusable CI
  gate and both complete wheel builds now precede either publish job, so a failing
  default or teleop leg publishes neither exact-pinned distribution.
