...
1. **Keep this file up to date.** `AGENTS.md` must always reflect the actual
...
2. **Maintain [CHANGELOG.md](CHANGELOG.md).** A comprehensive, human-readable changelog
   ...
   with a changelog entry in the same commit series. Only after version publication
   is confirmed, archive the finished notes with version and date at
   `docs/changelogs/CHANGELOG-<version>.md`, then reset root `CHANGELOG.md` to carry
   `[Unreleased]` plus an archive index. A tag or failed publish does not authorize
   archiving or clearing pending notes. Preserve every older archive, and commit
   the cleanup without rewriting a published tag.
...
4. **Git lives here.** The repository root is `waddle-sdk/`. Never `git init` or write
   artifacts outside this repository. Scratch work belongs in an ignored build
   directory or session scratchpad. `AGENTS.md` is canonical; keep `CLAUDE.md`
...
`waddle-protocol/docs/rationale/waddle_api_design_doc.md`. The normative documents
are `waddle-protocol/docs/{GLOSSARY,FSM,VERSIONING}.md`; they take precedence over
that informative rationale.

The SDK owns hardware and camera lifecycle, public observations/actions, hard
envelopes, supervision, recordings, and media transport. It remains independently
usable through public interfaces. Customer applications own their orchestration
and interfaces above this boundary; no application package is an SDK dependency.
...
reserved for the fully canonical profile. The `pick_lift` development distribution
uses +/-40 mm in each tabletop axis and +/-10 degrees yaw so repeated grasp trials
cannot assume the canonical cube pose; preserve its native reset-clearance and
planner-reachability checks when changing those bounds. Reference MuJoCo task
engines also retain bounded per-physics-step contact summaries for the current
evaluator episode so a slower hidden-state sampler can score transient contact and
peak-force events. Clear those summaries on every reset and never expose them
through participant observations, capabilities, or tools.
...
waddle-sdk/
  ...
  sdk/                       # the Python `waddle-sdk` frontend (PyO3 + maturin)
    ...
    python/waddle_sdk/       # THE IMPORTABLE PACKAGE IS `waddle_sdk`, not
                             #   `waddle`; the import matches the distribution.
                             ...
                             #   the canonical robot action descriptor so applications can map
                             #   named parts; schemas/
                             ...
                             #   Applications still see only SdkRuntimePort and the
                             ...
                             #   max velocity, not the arm-joint value, so callers
                             ...
                             #   There is no SDK-local UI or hosted task/artifact
                             #   facade; applications own those surfaces.
...
- The Python SDK runs from `sdk/` (Python 3.10+, `uv` on PATH):
  - `uv sync --dev --extra mujoco && uv run --no-sync pytest` — full build (maturin backend into `.venv`)
    + the pytest suite. The full scene-compiler suite requires the optional MuJoCo
...
- Commit style: small, coherent commits; imperative subject; body explains why. End
  commit messages with truthful attribution when applicable. Work on a feature
  branch, preserve existing history, and use repository-local identity
  `Vincent <neutrinovs@gmail.com>` unless the user specifies another identity.
  Do not push, publish packages, or deploy without authorization.
