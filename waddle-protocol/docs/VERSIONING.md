...
| `waddle.v0.calibration.measurements` | Local RGB-D click resolution and virtual-cell calibration lifecycle: `CalibrationMeasurement` (`GateClientMessage` arm 7) carries one bounded 3-D point, frame identity, and paired session timestamp; no RGB or depth image crosses the control plane. `CalibrationUpdate` (`GateServerMessage` arm 10) carries bounded collection/solve/catalog lifecycle. The virtual cell owns pose validation, motion, collection, solving, and customer-scoped catalog writes. Motion reuses ordinary `ActionChunk` intake, including whole-command refusal and the owner envelope; the feature introduces no alternate motion path. Requests are connection-scoped. |
