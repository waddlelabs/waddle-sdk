...
| `waddle.v0.agent.chat` | Authenticated UI chat with the active invited host: `ChatRequest` (`GateClientMessage` arm 5) and `ChatEvent` (`GateServerMessage` arm 8). A request carries bounded UTF-8 text and an opaque request ID; ordered events carry that ID, a sequence, and one of `accepted`, `text`, `done`, `unavailable`, or `error`. `text` is main-conversation assistant text only; implementations filter thinking, prompts, tool names and payloads, delegated output, and internal errors at both host and plane boundaries. One turn may be outstanding per session and only while its invited host is alive. This flag is connection-scoped: requests are never buffered offline and never replayed onto a connection that did not accept the flag. Chat adds no authority and no control behavior; its failure cannot disable local state, e-stop, jog, camera, or recording endpoints. |
| `waddle.v0.task.sessions` | Named durable hosted-task conversations: `TaskSessionRequest` (`GateClientMessage` arm 6) and `TaskSessionEvent` (`GateServerMessage` arm 9). CREATE supplies a bounded human-readable name and initial text; MESSAGE, INTERJECT, and INTERRUPT address the plane-issued session id. HISTORY requests the durable suffix strictly after `after_sequence`; each response is bounded to 256 stored events and ends with HISTORY_COMPLETE carrying the durable `history_cursor` for the next page. Ordered events expose only main-conversation text and public-safe lifecycle. Requests are connection-scoped and never replayed automatically; the durable plane transaction de-duplicates a fresh mutation request id after reconnect. The same hosted task record backs `waddle_sdk.agent(prompt)` and UI conversations; this flag adds lifecycle/persistence, never claim or lease authority. |
| `waddle.v0.calibration.measurements` | Local RGB-D click resolution and virtual-cell calibration lifecycle: `CalibrationMeasurementRequest` (`GateServerMessage` arm 13) asks the customer SDK to resolve one pixel against an exact retained frame; `CalibrationMeasurement` (`GateClientMessage` arm 7) returns the correlated bounded 3-D point, frame identity, and paired session timestamp. Neither RGB nor depth images cross the control plane. `CalibrationUpdate` (`GateServerMessage` arm 10) carries bounded collection/solve/catalog lifecycle. The virtual cell owns pose validation, motion, collection, solving, and customer-scoped catalog writes. Motion reuses ordinary `ActionChunk` intake, including whole-command refusal and the owner envelope; the feature introduces no alternate motion path. Both request and response are connection-scoped and are never buffered or automatically replayed. |
| `waddle.v0.workspace.artifacts` | Reviewed workspace delivery metadata: `WorkspaceArtifactRequest` (`GateClientMessage` arm 8) selects allowlisted graphs and calibration names; `WorkspaceArtifactReady` (`GateServerMessage` arm 11) returns only bounded identity, SHA-256, size, expiry, and an opaque one-time download reference. Archive bytes use a separate authenticated endpoint, never GateActions. Export begins empty and excludes prompts, skills markdown, MCP definitions, traces, secrets, and authoring metadata. Requests are connection-scoped. |
| `waddle.v0.connector.binding` | Exact connector authorization on `RegisterRequest`: `customer_id` (field 7), the existing `project` (field 1), `workspace_id` (field 8), and `authorization_only` (field 9). The three identifiers are all present or all absent. Every connector RPC also carries that same tuple as the non-secret `x-waddle-{customer,project,workspace}-id` metadata headers, plus `x-waddle-session-nonce` equal to the fresh UUID-v4 lowercase-hex `RegisterRequest.session_nonce`; the nonce rotates on reconnect. The plane validates the bearer credential, tuple, and nonce before accepting this flag. Register is a barrier: eagerly established streams carry no messages and neither endpoint processes non-Register traffic until the matching response is accepted. An authorization-only registration is connection-scoped, creates no runnable SDK session, makes no incomplete hardware declaration authoritative, accepts no hosted-run request, and exists so a customer connector can authenticate before opening hardware. A connector MUST fail closed unless the current `RegisterResponse` accepts this flag; reconnect authenticates again and never resumes motion. |
| `waddle.v0.hosted.runs` | Plane-started ordinary episodes for an otherwise idle remote SDK: `HostedRunRequest` (`GateServerMessage` arm 12) carries a bounded idempotency key, generic task metadata, and a positive relative timeout; `HostedRunStatus` (`GateClientMessage` arm 9) answers ACCEPTED, REJECTED, or BUSY with structured public-safe detail. Both directions are connection-scoped and never buffered or automatically replayed. A duplicate request ID within one SDK session returns its original admission answer verbatim and never opens a second episode. ACCEPTED opens through the ordinary reset/recording path and enters RUNNING via E6; all later authority, action, completion, recording, and fault behavior is existing v0 behavior. Expiry or loss of the accepting connection requests the core-owned energized HOLD path and terminates the episode with ABORT. Reconnect never resumes or replays motion; a host that still needs the admission result must retry explicitly by request ID. |
...
| `waddle.v0.motion.feedforward` | Optional `Action.joint_velocity_feedforward` (control.proto field 12) on an absolute joint-position step. The vector has the same declared order, width, and part scope as the position target; it is a driver tracking hint, never a second target or authority path. The core preserves it through direct substitution, bypass dispatch, and raw action recording. An actually interpolated blend drops it because changing the target path makes the sender's old velocity untruthful; an anchorless blend passes the target through unchanged and may retain it. A negotiated wrong-width, non-finite, gripper/noop, or non-position use refuses the whole chunk with the ordinary validation fault. **Pre-flag:** ignore the field completely and execute the position target unchanged, including when the ignored vector itself is malformed. The SDK declares support unconditionally because preserving an optional hint is hardware-neutral; a custom driver consumes it by implementing the public optional position+velocity driver protocol, while a position-only driver degrades explicitly to the unchanged target. No FSM guard row changes. The conformance surfaces are `intervention_chunk`, `expect_output.velocity_feedforward`, and `expect_send.velocity_feedforward`; fixture `agent_joint_velocity_feedforward_substitute` pins propagation. The payload adds one same-width low-rate vector to an existing control message and carries no media. |
